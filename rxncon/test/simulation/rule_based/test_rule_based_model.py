...
def test_association_specification():
    ...
    assert association_specification_occupied.association_definition.domain == 'AssociationDomain'
    assert association_specification_occupied.is_occupied

    assert association_specification_not_occupied.association_definition.domain == 'AssociationDomain'
...
def test_molecule_definition():
    ...
    assert molecule_definition.modification_definitions == mod_defs
    assert molecule_definition.association_definitions == ass_defs
    assert molecule_definition.localization_definition == loc_def


def test_molecule_specification(modification_definitions, association_definitions, localization_definition  ):
...
def test_complex_reactant_parts_different_location(molecules_bound):
    ...
    localization_specifications_raise = [rbm.LocalizationSpecification(localization_definition, 'Nucleus')]
...
def molecules_unbound(molecules_bound):
    ...
    molecule_definition_A = rbm.MoleculeDefinition('A', modification_definitions, association_definitions, localization_definition)
    molecule_specification_A_unbound = rbm.MoleculeSpecification(molecule_definition_A, molecule_specification_A_bound.modification_specifications, association_specification_A_unbound, molecule_specification_A_bound.localization_specifications)

    molecule_definition_B = rbm.MoleculeDefinition('B', [], association_definitions, localization_definition)
    molecule_specification_B_unbound = rbm.MoleculeSpecification(molecule_definition_B, [], assocociation_specification_B_unbound, molecule_specification_B_bound.localization_specifications)
