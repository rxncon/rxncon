...
def test_string_from_modification_specification():
    ...
    modification_specification = rxncon.semantics.molecule.ModificationProperty(modification_definition, 'P')

    mod_spec_str = bex.string_from_mod_prop(modification_specification)
...
def test_string_from_association_definition():
    ...
    assoc_def_str = bex.string_from_ass_prop_def(association_definition)
...
def test_string_from_association_specification():
    ...
    assert bex.string_from_ass_prop(rxncon.semantics.molecule.AssociationProperty(ass_def, rxncon.semantics.molecule.OccupationStatus.occupied_unknown_partner)) == \
        'AssociationDomain!+'

    assert bex.string_from_ass_prop(rxncon.semantics.molecule.AssociationProperty(ass_def, rxncon.semantics.molecule.OccupationStatus.not_occupied)) == \
        'AssociationDomain'

    assert bex.string_from_ass_prop(rxncon.semantics.molecule.AssociationProperty(ass_def, rxncon.semantics.molecule.OccupationStatus.not_specified)) == \
        'AssociationDomain!?'

    # This should not appear outside of a ComplexReactant.
    with pytest.raises(NotImplementedError):
        bex.string_from_ass_prop(rxncon.semantics.molecule.AssociationProperty(ass_def, rxncon.semantics.molecule.OccupationStatus.occupied_known_partner))
...
def test_string_from_localization_definition():
    ...
    loc_str = bex.string_from_loc_prop_def(localization_definition)
...
def test_string_from_localization_specification():
    ...
    localization_specification = rxncon.semantics.molecule.LocalizationProperty(localization_definition, 'Cell')
    loc_str = bex.string_from_loc_prop(localization_specification)

    assert loc_str == 'loc~Cell'


def test_string_from_molecule_definition(simple_molecule_spec):
    assert bex.string_from_mol_def(simple_molecule_spec.molecule_def) == \
...
def test_string_from_molecule_specification(simple_molecule_spec):
    assert bex.string_from_molecule(simple_molecule_spec) == \
...
def test_string_from_complex_reactant(simple_molecule_spec):
    ...
    bound_molecule_b = rxncon.semantics.molecule.Molecule(
            ...
            [],
            [rxncon.semantics.molecule.AssociationProperty(
            ...
            rxncon.semantics.molecule.LocalizationProperty(
    ...
    complex_reactant = rbm.ComplexReactant(
            ...
            [rxncon.semantics.molecule.Bond(left_spec=(0, bound_molecule_a.association_specs[0]), right_spec=(1, bound_molecule_b.ass_props[0]))]
    )
...
def simple_molecule_spec():
    ...
    mod_specs = [rxncon.semantics.molecule.ModificationProperty(mod_defs[0], 'P')]
    ...
    ass_specs = [rxncon.semantics.molecule.AssociationProperty(ass_defs[0], rxncon.semantics.molecule.OccupationStatus.not_occupied)]

    loc_def   = rxncon.semantics.molecule.LocalizationPropertyDefinition(['Cytosole', 'Nucleus'])
    loc_spec  = rxncon.semantics.molecule.LocalizationProperty(loc_def, 'Cytosole')

    molecule_definition = rxncon.semantics.molecule.MoleculeDefinition('A', mod_defs, ass_defs, loc_def)

    molecule_specification = rxncon.semantics.molecule.Molecule(molecule_definition, mod_specs, ass_specs, loc_spec)
...
def simple_rule_based_model():
    ...
    mol_spec_a_unbound = rxncon.semantics.molecule.Molecule(mol_def_a, [], [
        rxncon.semantics.molecule.AssociationProperty(assoc_def_a, rxncon.semantics.molecule.OccupationStatus.not_occupied)],
                                                            None)
    mol_spec_b_unbound = rxncon.semantics.molecule.Molecule(mol_def_b, [], [
        rxncon.semantics.molecule.AssociationProperty(assoc_def_b, rxncon.semantics.molecule.OccupationStatus.not_occupied)],
                                                            None)

    mol_spec_a_bound = rxncon.semantics.molecule.Molecule(mol_def_a, [], [
        rxncon.semantics.molecule.AssociationProperty(assoc_def_a, rxncon.semantics.molecule.OccupationStatus.occupied_known_partner)],
                                                          None)
    mol_spec_b_bound = rxncon.semantics.molecule.Molecule(mol_def_b, [], [
        rxncon.semantics.molecule.AssociationProperty(assoc_def_b, rxncon.semantics.molecule.OccupationStatus.occupied_known_partner)],
                                                          None)

    reactant_a_unbound = rbm.MoleculeReactant(mol_spec_a_unbound)
    reactant_b_unbound = rbm.MoleculeReactant(mol_spec_b_unbound)

    binding = rxncon.semantics.molecule.Bond((0, mol_spec_a_bound.ass_props[0]),
                                             (1, mol_spec_b_bound.ass_props[0]))
    ...
    rule = rxncon.semantics.rule.Rule([reactant_a_unbound, reactant_b_unbound], [reactant_ab], rxncon.semantics.rule.Arrow.reversible, rates)
