...
def target_from_str(target_str: str):
    ...
    """
    try:
        # reaction variations are specified by # followed by a number.
        if '#' in target_str:
...
def test_insulin_system():
    """
    Testing a system of 27 reactions, 9 state contingencies and 10 boolean contingencies.

    Note:
        The system contains 66 rules.
...
def test_deg_with_contingency():
    """
    Testing a degradation reaction with contingencies.

    Note:
        A system of 4 reactions and 1 contingency is translated into 12 rules.
...
def test_deg_with_interaction():
    ...
    expected_rules = {
        'C_deg_A#0,1': '( C & A_[x]--B_[y] & {} )'.format(A),
        'C_deg_A': '( C & {} )'.format(A),
        'A_[x]_ppi+_B_[y]': '( {0} & {1} )'.format(A, B),
        'A_[x]--0': '( A_[x]--0 & {} & ~(( C_deg_A | C_deg_A#0,1 )) & ~(( B_[y]--0 & A_[x]--0 & A_[x]_ppi+_B_[y] )))'.format(A),
        'B_[y]--0': '( {} & (( C_deg_A#0,1 & A_[x]--B_[y] ) | ( B_[y]--0 & ~(( A_[x]--0 & A_[x]_ppi+_B_[y] & B_[y]--0 )))))'.format(B),
        'A_[x]--B_[y]': '( {0} & {1} & (( B_[y]--0 & A_[x]--0 & A_[x]_ppi+_B_[y] ) | ( A_[x]--B_[y] & ~(( A_[x]--B_[y] & C_deg_A#0,1 )))) & ~(( C_deg_A | C_deg_A#0,1 )))'.format(A,B),
        'C': 'C'
    }
