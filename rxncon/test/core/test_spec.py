...
from rxncon.core.spec import Locus, MolSpec, DNASpec, MRNASpec, ProteinSpec, EmptyMolSpec, mol_spec_from_str, locus_from_str, \
...
def test_unstructured_specs():
    protein_spec = mol_spec_from_str('A_[dd/ss(rr)')
    # Protein
    ...
    empty_spec = mol_spec_from_str('0')
    assert isinstance(empty_spec, EmptyMolSpec)


def test_structured_specs():
    protein_spec = mol_spec_from_str('A@0_[dd/ss(rr)')
    # Protein
    assert isinstance(protein_spec, ProteinSpec)
    assert protein_spec.has_resolution(LocusResolution.residue)
    assert protein_spec.to_component_spec().has_resolution(LocusResolution.component)
    ...
    with pytest.raises(SyntaxError):
        empty_spec = mol_spec_from_str('0@1')


def test_bond_specs():
    bond_spec = bond_spec_from_str('A_[x]~B_[y]')
    assert bond_spec.first == mol_spec_from_str('A_[x]')
    assert bond_spec.second == mol_spec_from_str('B_[y]')

    assert bond_spec_from_str('B_[y]~A_[x]') == bond_spec_from_str('A_[x]~B_[y]')

    assert bond_spec_from_str('A_[x]~0') == mol_spec_from_str('A_[x]')
...
def test_super_sub():
    ...
    assert mol_spec_from_str('A_[dom]').is_subspec_of(mol_spec_from_str('A'))
    assert not mol_spec_from_str('A').is_subspec_of(mol_spec_from_str('A_[dom]'))


def test_equality():
    non_struct = mol_spec_from_str('A_[d/s(r)]')
    struct     = mol_spec_from_str('A@5_[d/s(r)]')
