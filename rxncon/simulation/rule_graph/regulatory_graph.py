...
class RegulatoryGraph():
    ...
    def add_contingency_information_to_graph(self, contingencies: List[Contingency]) -> None:
        for contingency in contingencies:
            self.add_information_from_effector_to_graph(contingency.effector, contingency.type,
                                                        self._target_name_from_reaction_or_effector(contingency.target)
                                                        )

    def replace_invalid_chars(self, name: str) -> str:
        return re.sub('[<>]', '', name)

    def _add_node_and_edge(self, name: str, node_type: NodeType, edge_type: EdgeInteractionType, target_name: str) -> None:
        self.graph.add_node(self.replace_invalid_chars(str(name)),
                            type=node_type.value)

        self.graph.add_edge(self.replace_invalid_chars(str(name)), target_name,
    ...
    def add_information_from_effector_to_graph(self, effector, edge_type, target_name) -> None:
        if isinstance(effector, StateEffector):
            ...
            self.add_information_from_effector_to_graph(effector.expr, EdgeInteractionType.NOT, target_name)
            ...
            self.add_information_from_effector_to_graph(effector.left_expr, EdgeInteractionType.OR, target_name)
            self.add_information_from_effector_to_graph(effector.right_expr, EdgeInteractionType.OR, target_name)
            ...
            self.add_information_from_effector_to_graph(effector.left_expr, EdgeInteractionType.AND, target_name)
            self.add_information_from_effector_to_graph(effector.right_expr, EdgeInteractionType.AND, target_name)
        else:
            raise AssertionError
