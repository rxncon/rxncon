...
def rule_based_model_from_rxncon(rxnconsys: rxs.RxnConSystem) -> rbm.RuleBasedModel:
    molecule_defintions = molecule_defs_from_rxncon(rxnconsys)
...
@tc.typecheck
def name_to_assoc_defs_from_state(name_to_assocication_definitions: dict, state: tc.any(sta.InterProteinInteractionState, sta.IntraProteinInteractionState)):
    ...
    name_to_assocication_definitions[state.first_component.name].add(rbm.AssociationDefinition(first_domain))
    name_to_assocication_definitions[state.second_component.name].add(rbm.AssociationDefinition(second_domain))

    return name_to_assocication_definitions
